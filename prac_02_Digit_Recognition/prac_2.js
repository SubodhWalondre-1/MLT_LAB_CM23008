        ...
        async function trainModel() {
            ...
            try {
                ...
                for (let epoch = 0; epoch < epochs; epoch++) {
                    ...
                    for (let batch = 0; batch < trainBatchesPerEpoch; batch++) {
                        ...
                        const history = await model.fit(
                            trainBatch.xs, trainBatch.labels,
                            {
                                batchSize: batchSize,
                                epochs: 1,
                                verbose: 0
                        ...
                        if ((batch + 1) % 10 === 0) {
                            console.log(`  Batch ${batch + 1}/${trainBatchesPerEpoch} - Progress: ${progress.toFixed(1)}%`);
                        }
                    }
                    ...
                    document.getElementById('trainAccuracy').textContent = (avgAcc * 100).toFixed(2) + '%';
                ...
                document.getElementById('status').textContent = '✅ Model trained successfully! Draw a digit or evaluate on test set.';
        ...
        async function evaluateModel() {
            ...
            document.getElementById('status').textContent = 'Evaluating model on test set...';
            ...
            try {
                ...
                for (let i = 0; i < numTestBatches; i++) {
                    ...
                    console.log(`✓ Batch ${i + 1}/${numTestBatches} - Correct: ${correct[0]}/${testBatchSize} (${((correct[0]/testBatchSize)*100).toFixed(2)}%)`);
                ...
                console.log(`✓ Correct Predictions: ${totalCorrect}/${numTestBatches * testBatchSize}`);
                console.log(`✗ Incorrect Predictions: ${(numTestBatches * testBatchSize) - totalCorrect}`);
                console.log('='.repeat(60) + '\n');

                document.getElementById('testAccuracy').textContent = (testAcc * 100).toFixed(2) + '%';
                ...
                console.error('Error stack:', error.stack);
                document.getElementById('status').textContent = '❌ Error evaluating: ' + error.message;
                document.getElementById('evaluateBtn').disabled = false;
            }
        }
        ...
        async function showTestSamples() {
            ...
            for (let i = 0; i < numSamples; i++) {
                ...
                console.log(`Sample ${i + 1}: True=${trueClass}, Predicted=${predClass} ${isCorrect ? '✓' : '✗'}`);
                ...
                labelDiv.className = 'test-label ' + (predClass === trueClass ? 'correct' : 'incorrect');
        ...
        async function predictDigit() {
            try {
                ...
                console.log(`💯 Confidence: ${(probabilities[predictedClass[0]] * 100).toFixed(2)}%`);
                ...
                console.log(Array.from(probabilities).map((p, i) => `${i}: ${p.toFixed(6)}`).join(', '));
                console.log('='.repeat(60) + '\n');
